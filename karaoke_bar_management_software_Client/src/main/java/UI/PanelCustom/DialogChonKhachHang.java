...
public class DialogChonKhachHang extends JDialog
	...
	private KhachHang customer = null;
	...
	public void createUI() {
		...
		try {
			...
			LoadCustomerList(customerDAO.getCustomerListUnBooked());
		} catch (Exception e) {
			e.printStackTrace();
		}
	}
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (o.equals(btnSearch)) {
			...
			if (maKH.equalsIgnoreCase("")) {
				...
				try {
					KhachHangDAO customerDAO = (KhachHangDAO) Naming.lookup("rmi://localhost:1099/customerDAO");
					customer = customerDAO.getCustomerById(maKH);
					LoadCustomerList(customerDAO.getCustomerListUnBooked());
				} catch (Exception e1) {
					e1.printStackTrace();
				}
	...
	private void loadBtnCustomer(String customerId) {
		...
		String btnName = "<html>" + "<p style='text-align: left; width:116px'>Mã KH: " + customer.getMaKH() + " </p>"
		...
		for (int i = 0; i < sizeDSPhong; i++) {
			...
			btnCustomerList[selection].addActionListener(new ActionListener() {
				...
				public void actionPerformed(ActionEvent e) {
					...
					KhachHang customerActiveE = new KhachHang();
					try {
						KhachHangDAO customerDAO = (KhachHangDAO) Naming.lookup("rmi://localhost:1099/customerDAO");
						customerActiveE = customerDAO.getCustomerById(maKH);
					} catch (Exception e1) {
						e1.printStackTrace();
					}
					loadDataLenForm(customerActiveE);
				}
			});
			// sự kiện hover chuột
	...
	private void searchCustomer() {
		...
		try {
			...
			switch (searchTypeName) {
				case "Tất cả":
					txtKeyword.setText("");
					customerList = customerDAO.getCustomerListUnBooked();
					break;
				case "Tên Khách hàng":
					customerList = customerDAO.getCustomerListUnBookedByName(keyword);
					break;
				case "CMND/CCCD":
					customerList = customerDAO.getCustomerListUnBookedByCMND(keyword);
					break;
				case "Số điện thoại":
					customerList = customerDAO.getCustomerListUnBookedByPhoneNumber(keyword);
					break;
			}
		} catch (Exception e) {
			e.printStackTrace();
