...
public class PnDatPhong extends JPanel
	...
	public PnDatPhong(NhanVien staff) {
		...
		btnRentRoom.setBounds(191, 150, 120, 35);
		...
		btnPayment.setBounds(339, 150, 120, 35);
		...
		btnChooseCustomer.setBounds(28, 150, 135, 35);
	...
	public void allLoaded() {
		...
		ArrayList<Phong> roomList = roomDAO.getRoomList();
	...
	private void loadRoom(String maPhong) {
		Phong room = roomDAO.getRoomByRoomId(maPhong);
		if (room == null)
			room = new Phong();
		...
		for (int i = 0; i < sizeDSPhong; i++) {
			...
			btnRoomList[selection].addActionListener(new ActionListener() {
				...
				public void actionPerformed(ActionEvent e) {
					...
					String roomTypeName = roomTypeDAO.getRoomTypeNameById(roomID);
					...
					HoaDon bill = billDAO.getUncheckBillByRoomId(roomID);
					if (bill != null) {
					...
					spnOrderQuantity.setValue((int) 1);
					Phong roomActiveE = roomDAO.getRoomByRoomId(roomID);
					if (roomActiveE == null)
	...
	private void showBillInfo(String maPhong) {
		ArrayList<CTDichVu> dataList = serviceDetailDAO.getServiceDetailListByRoomId(maPhong);
		int i = 1;
	...
	private void loadCboRoomType() {
		ArrayList<LoaiPhong> dataList = roomTypeDAO.getRoomTypeList();
	...
	private void loadDSServiceType() {
		ArrayList<LoaiDichVu> serviceTypeList = serviceTypeDAO.getServiceTypeList();
	 ...
	 */
	private String createNewBillId(Timestamp date) {
		SimpleDateFormat format = new SimpleDateFormat("yyyyMMdd");
		String dateStr = format.format(date);
		String lastBillId = billDAO.getLastBillId();
		...
		if (txtBillID.getText().trim().equals("")) {
			...
			if (quantityInStock <= 0) {
				...
				if (!billID.equalsIgnoreCase("")) {
					CTDichVu serviceDetail = serviceDetailDAO.getServiceDetailByBillIdAndServiceId(billID,
							service.getMaDichVu());
